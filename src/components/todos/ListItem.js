    ...
    render() {
        ...
        return (
            <div onDoubleClick={(e) => {editTodo(todo)}} className="row row--list-item">
                <input className="list-item__checkbox" type="checkbox" onDoubleClick={e => e.stopPropagation()} onChange={(e) => actions.updateTodoChecked(e.target.checked, todo.get('id'))} checked={todo.get('completed')}/>
                {editable ?
                <input className="edit-item" autoFocus="true" type="text" onBlur={this.onBlur} onKeyDown={this.onKeyDown} onChange={(e) => updateTodo(e.target.value)} value={editText}/>
                :
